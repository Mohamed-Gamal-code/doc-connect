...
export default function FindDoctors() {
  // 1. استخراج البيانات من الـ URL
  const searchParams = useSearchParams();
  const initialQuery = searchParams.get("query") || "";
  const initialLocation = searchParams.get("location") || "";

  // 2. الحالات (States) - بنحط فيها القيم اللي جاية من الـ URL كبداية
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const [locationQuery, setLocationQuery] = useState(initialLocation);
  const [selectedSpecialty, setSelectedSpecialty] = useState("All Specialties");

  // 3. منطق الفلترة (العقل المدبر)
  const filteredDoctors = doctors.filter((doctor) => {
    // هل الاسم أو التخصص فيه الكلام اللي مكتوب في البحث؟
    const matchesSearch =
      doctor.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      doctor.specialty.toLowerCase().includes(searchQuery.toLowerCase());

    // هل عنوان الدكتور فيه الكلام اللي مكتوب في الـ Location؟
    const matchesLocation = doctor.location
      .toLowerCase()
      .includes(locationQuery.toLowerCase());

    // هل التخصص هو اللي مختارينه من الـ Sidebar؟
      ...
      <div className="container mx-auto px-6 lg:px-12 mt-12">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <aside className="hidden lg:block space-y-8">
            ...
            <div>
              <label className="text-sm font-black text-slate-700 block mb-3">
                Specialty
              </label>
              <select
          ...
          <div className="lg:col-span-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredDoctors.map((doctor) => (
                <Card
                  key={doctor.id}
                  className="group border-none shadow-[0_4px_20px_-10px_rgba(0,0,0,0.1)] hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.1)] transition-all duration-500 rounded-[2rem] overflow-hidden bg-white"
                >
                  {/* ... نفس كود كارت الدكتور اللي عملناه قبل كدة ... */}
                  <div className="flex flex-col sm:flex-row h-full">
                    <div className="sm:w-2/5 relative h-64 sm:h-auto overflow-hidden">
                      ...
                      />
                      <div className="absolute top-4 left-4 bg-emerald-600 text-white px-3 py-1 rounded-xl text-[10px] font-black uppercase">
                        {doctor.fee}
                      </div>
                    </div>
                    ...
                    <CardContent className="p-6 flex-1 flex flex-col justify-between">
                      ...
                      <CardFooter className="p-0 mt-6 pt-6 border-t border-slate-50">
                        <Link
                          ...
                          className="w-full"
                        >
                          <Button className="w-full bg-slate-900 hover:bg-emerald-600 text-white rounded-2xl py-6 font-black text-xs uppercase tracking-widest transition-all">
                            Book Appointment
                          </Button>
                        </Link>
                      </CardFooter>
